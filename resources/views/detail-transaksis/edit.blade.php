...
<div class="card" style="max-width: 680px;">
    <div class="card-header">
        <span class="card-title">✏️ Form Edit Detail Transaksi</span>
    ...
    <div class="card-body">
        ...
        <form action="{{ route('detail-transaksis.update', $detail->id) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <select name="transaksi_id" id="transaksi_id" class="form-control" required>
                    ...
                    @foreach($transaksis as $transaksi)
                        <option value="{{ $transaksi->id }}"
                            {{ old('transaksi_id', $detail->transaksi_id) == $transaksi->id ? 'selected' : '' }}>
                ...
                <select name="mobil_id" id="mobil_id" class="form-control" required onchange="hitungSubtotal()">
                    ...
                    @foreach($mobils as $mobil)
                        <option value="{{ $mobil->id }}"
                            ...
                            {{ old('mobil_id', $detail->mobil_id) == $mobil->id ? 'selected' : '' }}>
            ...
            <div class="grid-2">
                ...
                <div class="form-group">
                    <label class="form-label">Estimasi Subtotal Baru</label>
                    <input type="text" id="preview_subtotal" class="form-control"
                        value="Rp {{ number_format($detail->subtotal, 0, ',', '.') }}"
                        readonly style="background:#f8fafc; font-weight:700; color:var(--primary);">
                </div>
            </div>
