...
@section('page-title', 'Edit Detail Transaksi')
@section('page-subtitle', 'Perbarui item detail transaksi')

@section('content')
{{-- Menampilkan form edit detail transaksi untuk memperbarui data transaksi --}}
<div class="row">
    <div class="col-lg-8">
        <div class="card shadow mb-4">
            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                <h6 class="m-0 font-weight-bold text-primary">
                    <i class="fas fa-edit mr-2"></i>Form Edit Detail Transaksi
                </h6>
            ...
            <div class="card-body">
                ...
                <form action="{{ route('detail-transaksis.update', $detail->id_detail) }}" method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <select name="transaksi_id" id="transaksi_id" class="form-control" required>
                            ...
                            @foreach($transaksis as $transaksi)
                                <option value="{{ $transaksi->id_transaksi }}"
                                    {{ old('transaksi_id', $detail->transaksi_id) == $transaksi->id_transaksi ? 'selected' : '' }}>
                        ...
                        <select name="mobil_id" id="mobil_id" class="form-control" required onchange="hitungSubtotal()">
                            ...
                            @foreach($mobils as $mobil)
                                <option value="{{ $mobil->id_mobil }}"
                                    ...
                                    {{ old('mobil_id', $detail->mobil_id) == $mobil->id_mobil ? 'selected' : '' }}>
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            ...
                            <input type="number" name="jumlah_beli" id="jumlah_beli" class="form-control"
                                value="{{ old('jumlah_beli', $detail->jumlah_beli) }}" min="1" required onchange="hitungSubtotal()">
                        </div>
                        <div class="form-group col-md-6">
                            <label class="font-weight-bold small text-uppercase text-muted">Estimasi Subtotal Baru</label>
